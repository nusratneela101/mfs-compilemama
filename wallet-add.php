...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($csrf)) {
        ...
        $result = addMoney($userId, $amount, $mfsProvider['name'], $mfsAccount);
        if ($result['success']) {
            $success = $result['message'];
            $txRef   = $result['reference_id'];
            $fee     = $result['fee'];
            $wallet  = getOrCreateWallet($userId); // refresh
        } else {
...
<div class="container py-4" style="max-width:600px">
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
            <h4 class="fw-bold mb-0">📥 অ্যাড মানি</h4>
            <small class="text-muted">যেকোনো MFS থেকে ওয়ালেটে টাকা যোগ করুন</small>
        </div>
    </div>
    ...
    <div class="stat-card text-center py-5">
        ...
        <div class="d-flex justify-content-center gap-3 mt-3">
            <a href="/wallet-add.php" class="btn btn-primary rounded-pill">আবার যোগ করুন</a>
    ...
    <form method="post" autocomplete="off">
        ...
        <div class="stat-card mb-3">
            <h6 class="fw-bold mb-3">MFS প্রদানকারী বেছে নিন</h6>
            <div class="row g-2">
        ...
        <!-- MFS Account Number -->
        <div class="stat-card mb-3">
            <label class="form-label fw-semibold">MFS অ্যাকাউন্ট নম্বর</label>
        ...
        <div class="stat-card mb-3">
            ...
            <!-- Fee Notice -->
            <div class="mt-3 p-2 rounded-2 small <?= $fee > 0 ? 'bg-warning bg-opacity-10' : 'bg-success bg-opacity-10' ?>">
                <?php if ($fee > 0): ?>
                ⚠️ এই লেনদেনে ৳<?= number_format($fee, 0) ?> সার্ভিস চার্জ প্রযোজ্য।
                <?php else: ?>
                🆓 এই লেনদেন ফ্রি! (আরও ৳<?= number_format(max(0, WALLET_FREE_LIMIT - $wallet['free_limit_used']), 0) ?> পর্যন্ত ফ্রি)
                <?php endif; ?>
            </div>
        </div>
