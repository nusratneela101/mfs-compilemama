    ...
    if (!verifyCSRFToken($token)) {
        ...
        if (empty($name) || strlen($name) < 2) {
            ...
            if ($dup) {
                ...
                if ($dupEmail) {
                    $error = 'এই ইমেইলটি ইতিমধ্যে নিবন্ধিত।';
                } else {
                    $hash = hashPIN($pin);
                    $stmt = $db->prepare(
                        "INSERT INTO users (phone, pin_hash, name, email, status) VALUES (?, ?, ?, ?, 'inactive')"
                    );
                    $stmt->bind_param('ssss', $phone, $hash, $name, $email);
                    if ($stmt->execute()) {
                        $userId = (int)$db->insert_id;
                        $stmt->close();

                        // Rate-limit verification email sends: max 3 per hour per email
                        if (!rateLimitCheck($email, 'email_verify', 3, 3600)) {
                            ...
                            $stmtV->bind_param('isss', $userId, $email, $verifyToken, $expiresAt);
                            $stmtV->execute();
                            $stmtV->close();

                            sendVerificationEmail($email, $name, $verifyToken);

                            $success = 'আপনার ইমেইলে একটি ভেরিফিকেশন লিংক পাঠানো হয়েছে। লিংকে ক্লিক করে অ্যাকাউন্ট অ্যাক্টিভ করুন।';
                        }
                    } else {
        ...
        <div class="row justify-content-center">
            <div class="col-sm-10 col-md-8 col-lg-6 col-xl-5">
                ...
                <div class="alert alert-success rounded-3">
                    <i class="fas fa-envelope me-2"></i><?= sanitize($success) ?>
                </div>
                <?php else: ?>
